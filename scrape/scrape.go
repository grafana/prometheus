...
func (c *scrapeCache) addRef(met string, ref storage.SeriesRef, lset labels.Labels, meta metadata.Metadata, hash uint64) *cacheEntry {
	// The cache entries are used for staleness tracking so even if ref is
	// 0 we need to track it.
	intern.Intern(c.interner, lset)

	ce := &cacheEntry{ref: ref, lastIter: c.iter, lset: lset, meta: meta, hash: hash}
	c.series[met] = ce
	return ce
}
...
loop:
	for {
		...
		if !ok {
			ce := sl.cache.addRef(mets, ref, lset, meta, hash)
			if tp == nil {
